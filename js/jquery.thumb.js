...
+function ($) {
    ...
    function Thumb() {
        ...
        this.display = function (config) {
			...
			//上传成功后的回调函数
			if(config.upload_success != undefined){
				this.upload_success=config.upload_success;
			}
			//处理成功后的回调函数
			if(config.process_success != undefined){
				this.process_success=config.process_success;
			}
		...
		this.upload_success=function(outer,data){
        ...
        this.uploadInfo = function () {
            ...
            $.ajax({
                ...
                //contentType: false,   // 如果data使用的是 FormData对象 则不需要自动添加头信息
                success: function (data) {
					outer.process_success(outer,data);
					/*
                    if(data.status==="0"){
                        alert("头像修改成功！");
                    }else{
		...
		this.process_success=function(outer,data){
			if(data.status==="0"){
				alert("头像修改成功！");
			}else{
				alert("出错了！"+data.error);
